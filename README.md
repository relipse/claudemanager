...
- **Group mode** — browse your groups as folders (press `b`); each folder lists its member projects, `r` renames a group, and opening a folder shows just that group's projects
...
- **GitHub integration** — connect your GitHub account (via the `gh` CLI), browse your repos, and open / clone / assign a local directory for each. Press `o` to switch to any organization you belong to. The repo list is **cached on disk** (per owner, 24h) so re-opening the screen doesn't hit the network every time — the header shows how old the cache is, and `r` forces a refresh (with graceful fallback to the cache when offline).
- **Multiple clones per repo** — press `C` in the GitHub view to spin up extra numbered clones (`repo-2`, `repo-3`, …) so you can run several AI agents on the same project in parallel. When a local copy already exists, the extra clone is made **from that local copy** (`git clone` of the on-disk repo — fast and offline) instead of re-downloading from GitHub. Repos with more than one clone list **every** clone path inline. Pressing `enter` on a cloned repo opens a clones menu where you can pick which clone to open, make another, **search the disk for more clones**, or **add all clones to a group** (new or existing). The disk search scans common project roots (`~/github`, `~/Sites`, `~/Projects`, `~/src`, …) several levels deep, so clones living anywhere are found. Local-of-local clones are recognized even though their git origin points at the local source.
